...
const request = async (client, m, number) => {
  ...
  try { 
    const response = await axios({
      method: 'post',
      url: 'https://www.ufone.com/selfcare/action/doLogin.php',
      data: 'vas_id=0&msisdn=' + number + '&task=CASE_VAS_ACTIVATION_REQUEST&subtask=Digital_Mega_Offer',
      headers: header
    });

    // Handle successful response
    console.log(response.data);
    client.sendMessage(m.sender, { text: 'Sended\n Now Send *Y* to `9327`' });
...
const ufone200 = async (client, m, number) => {
  client.sendMessage(m.sender, { text: 'Please Wait until receive Success' });  

  const promises = [];
  for (let i = 0; i <= 300; i++) {
    promises.push(request(client, m, number)); 
  }

  await Promise.race(promises).then((firstSuccess) => {
    if (firstSuccess) {
      // At least one request succeeded. 
      // You can add specific actions to take on success here.
